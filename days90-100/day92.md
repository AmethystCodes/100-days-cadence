## Day 92/100 Days of Cadence

* Updated contract with new interface and borrow function
```cadence
...
```
* Created script to verify an account is properly setup
```cadence
import Badger from 0x01
import NonFungibleToken from 0x02

pub fun main(account: Address): Bool {
    if getAccount(account).getCapability<&Badger.Collection{NonFungibleToken.CollectionPublic}>(Badger.BadgerCollectionPublicPath).borrow() == nil {
        return false
    }
    return true
} 

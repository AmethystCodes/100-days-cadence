## Day 89/100 Days of Cadence

* Updated contract with `delete` function
```cadence
...
* Delete badge transaction 
```cadence
import Badger from 0x01

transaction(id: UInt64) {

    let BadgeCollection: &Badger.Collection

    prepare(acct: AuthAccount) {
        self.BadgeCollection = acct.borrow<&Badger.Collection>(from: Badger.BadgerCollectionStoragePath)
                ?? panic("Could not borrow Collection from signer.") 
    }

    execute {
        self.BadgeCollection.delete(id: id)
        log("Badge Deleted.")
    }
}
```
